...
router.get('/daily/pdf', async (req, res, next) => {
  try {
    ...
    const orderRowsHtml = orders.length
      ...
      `).join('')
      : `<tr><td colspan="7" style="text-align:center; color:#6b7280;">No orders for this date</td></tr>`;
        ...
        <body>
          <div class="header">
            ...
            <div style="text-align:right;">
              <div class="muted"><strong>Date</strong></div>
              <div>${formatReportDate(start)}</div>
            </div>
          </div>
    ...
    const fileDate = start.toISOString().slice(0, 10);
    res.set({
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename=daily-report-${fileDate}.pdf`,
